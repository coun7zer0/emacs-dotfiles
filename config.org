...
** Core
*** Devdocs
Instala devdocs, una utilidad para ver la documentación de diversos
lenguajes dentro de Emacs.

#+begin_src emacs-lisp
  (use-package devdocs
    :ensure t
    :hook (devdocs-mode . (lambda () (setq truncate-lines nil)))
    :config
...
*** company-mode
Instala el paquete que nos dará el menu de opciones del
auto-completado.
...
*** Eglot
Configura Eglot, el cual es un cliente LSP.
...
#+begin_src emacs-lisp
  (use-package eglot
    :bind (:map eglot-mode-map
                ...
                ("C-c e s a" . eglot-shutdown-all)
                ("C-c e r" . eglot-rename)
                ("C-c e f" . eglot-format)
                ("C-c e p e" . flymake-show-project-diagnostics)
                ("C-c e b" . flymake-goto-prev-error)
                ("C-c e f" . flymake-goto-next-error))
    :custom-face
    ...
    (auto-shutdown t)
    :config
    ;; Esta funcion se utiliza para indicarle a eglot que no tome por completo
    ;; el control de los backends de flymake, asi de esta forma nosotros podamos utilizar backends extenos como el de eslint
...
*** eglot-booster
Se tiene que instalar [[https://github.com/blahgeek/emacs-lsp-booster][emacs-lsp-booster]] en nuestro sistema para que se
pueda ejecutar el siguiente código, este paquete hace que eglot funcione mas rápido.
...
*** eglot-x
Instala un paquete que le da soporte a extensiones no oficiales de
lsp.
...
*** Configuración
Eglot
Establece la configuración de Eglot al iniciar java.
  ...
  (with-eval-after-load 'eglot
    ...
    (let ((cache (expand-file-name (md5 (project-root (project-current t)))
      ...
      (add-to-list 'eglot-server-programs
                   `(java-ts-mode . ("/home/dotwilliamrc/.lsp-servers/jdtls/bin/jdtls"
                                     "configuration" "/home/dotwilliamrc/.lsp-servers/jdtls/config_linux"
                                     ,(concat "--jvm-arg=-javaagent:" (expand-file-name "/home/dotwilliamrc/.m2/repository/org/projectlombok/lombok/1.18.32/lombok-1.18.32.jar"))
                                     "-data" ,cache :initializationOptions
                                     (:settings
                                      (:java
                                       (:autobuild (:enable t)
                                                   :extendedClientCapabilities (:classFileContentSupport t)))))))))
#+end_src
...
*** Configuración Eglot
Establece la configuración de eglot al iniciar YAML.

#+begin_src emacs-lisp
  (with-eval-after-load 'eglot
    (add-to-list 'eglot-server-programs
                 '(yaml-ts-mode . ("node" "/home/dotwilliamrc/.lsp-servers/yaml-language-server/out/server/src/server.js" "--stdio"))))
#+end_src
...
*** Configuración Eglot
Establece la configuración de eglot al iniciar HTML.

#+begin_src emacs-lisp  
  (with-eval-after-load 'eglot
    (add-to-list 'eglot-server-programs
                 '(html-web-mode . ("vscode-html-language-server" "--stdio"))))
#+end_src
...
*** flymake-eslint
Instala flymake-eslint, el cual es un paquete que nos ayuda a que
flymake nos muestre las advertencias de eslint.
...
*** web-mode
Cambie al modo a usar en los archivos de tipo ".js" ".ts" ".tsx" y
"tsx".
...
*** Configuración Eglot
Establece la configuración de eglot al iniciar code-web-mode.

#+begin_src emacs-lisp  
  (with-eval-after-load 'eglot
    (add-to-list 'eglot-server-programs
                 '(code-web-mode . ("typescript-language-server" "--stdio"))))
#+end_src
...
** SQL
*** Conexión
Configura la conexión de MariaDB para SQLi.

#+begin_src emacs-lisp
  (setq sql-connection-alist
        '((mariadb-local (sql-product 'mariadb)
                        (sql-user (getenv "SQL_USER"))
                        (sql-password (getenv "SQL_PASSWORD"))
                        (sql-server (getenv "SQL_SERVER"))
                        (sql-database (getenv "SQL_DATABASE")))))
#+end_src

# *** SQLi
# Abre una instancia de SQLi una ventana contigua.

# #+begin_src emacs-lisp
#     (defun sql-connect-other-window ()
#     (unless (sql-find-sqli-buffer)
#       (let ((new-window (split-window-right)))
#         (select-window new-window)
#         (sql-connect 'mariadb-local)
#         (select-window (previous-window)))))

#   (add-hook 'sql-mode-hook 'sql-connect-other-window)
#   #+end_src

*** Inicio
Configura las funciones que inician con SQL-mode.

#+begin_src emacs-lisp
  (load-file (expand-file-name "lisp/company-sql.el" user-emacs-directory))

  (defun start-sql-modes ()
    (set-fill-column 70)
    (display-fill-column-indicator-mode)
    (company-mode)
    (company-sql)
  ...
  (add-hook 'sql-mode-hook 'start-sql-modes)
#+end_src

*** Comandos Personalizados
**** sqli-show-tables
Muestra las tablas de la base de datos de la conexión actual de SQLi.

#+begin_src emacs-lisp
  (defun cz/sqli-show-tables ()
  "Send 'show tables,' to the active SQLi buffer if it exist."
  (interactive)
  (if (sql-find-sqli-buffer)
      (sql-send-string "show tables;")
    (message "No active SQLi buffer found.")))
#+end_src

**** sqli-describe-table
Muestra los campos de la tabla-at-point y si no se encuentra nada
at-point pide el nombre de la tabla para buscarla.

#+begin_src emacs-lisp
  (defun cz/sqli-describe-table ()
  "Describe the SQL table at point or, if none is found, show tables and prompt for a table name."
  (interactive)
  (if (sql-find-sqli-buffer)
      (let ((table-name (thing-at-point 'symbol t)))
        (if table-name
            ;; Si hay algo en el punto, usarlo como nombre de la tabla
            (sql-send-string (format "describe %s;" table-name))
          ;; Si no hay nada en el punto, primero mostrar las tablas y luego pedir el nombre de la tabla
          (progn
            (sql-send-string "show tables;")
            (setq table-name (read-string "Enter table name: "))
            (sql-send-string (format "describe %s;" table-name)))))
    (message "No active SQLi buffer found.")))
#+end_src

**** Mapeo
Mapea los comandos anteriores solo al iniciar el sql-mode.

#+begin_src emacs-lisp
  (defun sql-mode-keybindings ()
    (define-key sql-mode-map (kbd "C-c e t") 'cz/sqli-show-tables)
    (define-key sql-mode-map (kbd "C-c e f") 'cz/sqli-describe-table))

  (add-hook 'sql-mode-hook 'sql-mode-keybindings)
#+end_src
